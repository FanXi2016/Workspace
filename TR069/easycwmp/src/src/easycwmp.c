...
#include "ubus.h"
#include "log.h"
#include "json.h" // Connie add for value change inform, 2014/7/7

#define DO_AFTER_REBOOT_FILE "/usr/share/easycwmp/oem/do_after_reboot" // Connie add for value change inform, 2014/7/7
...
int main (int argc, char **argv)
{
  ...
  // Connie add start for value change inform, 2014/7/7
  if (access(DO_AFTER_REBOOT_FILE,0) == 0)
  {
    char buf[1024] = {0};
    char *line;
    FILE *fp = NULL;
    ...
    D("Connie debug: start process do_after_reboot!\n");
    fp = fopen(DO_AFTER_REBOOT_FILE, "rb");

    if (NULL != fp)
    {
      read_size = fread(buf, 1, 1024, fp);
      if (read_size > 0)
      {
        D("Connie debug: buf: %s\n", buf);
        line = strtok(buf, "\n");
        while (line != NULL)
        {
          D("Connie debug: line: %s\n", line);
          json_handle_parser_line(line);
          line = strtok(NULL, "\n");
        }
      }
      fclose(fp);
    }
    system("rm /usr/share/easycwmp/oem/do_after_reboot");
  }
  // Connie add end for value change inform, 2014/7/7
