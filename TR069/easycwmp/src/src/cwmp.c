...
end_fault :
	log_message(NAME, L_NOTICE, "upload error: '%s'", fault_array[code].string);
	backup_update_fault_transfer_complete(node, code);
...
int cwmp_get_int_event_code(char *code)
{
	...
	//Added By Tian Yiqing, To deal with diagnostics complete event, Start
	...
	//Added By Tian Yiqing, To deal with diagnostics complete event, End
